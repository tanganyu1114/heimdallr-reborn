...
export class NewDirStreamProxyPassGenerator extends NewContextGenerator {
  constructor(data) {
    super(data)
    var formatData = {
      'enabled': true,
      'context-type': 'dir_stream_proxy_pass',
      'context-value': ''
    }
    this.data = undefined
...
export class DirStreamProxyPassCreator extends _ContextCreator {
  get camelCaseCtxName() { return 'ProxyPass(Stream)' }
  className() {
    return 'DirStreamProxyPassCreator'
  }
  defaultFormData() {
    return {
      proxyPassValue: ''
  ...
  formRules() {
    return {
      proxyPassValue: [
        { required: true, message: '请输入反向代理地址', type: 'string', trigger: 'blur' }
      ]
    }
  }
  ...
  generateSubmitOptions() {
    return new NewDirStreamProxyPassGenerator(this.formData).generateNewContextOptions()
  }
}

export default {
  name: 'DirStreamProxyPassCreator',
  ...
  data() {
    return {
      creator: new DirStreamProxyPassCreator()
    }
  },
  methods: {
...
<template>
  <div>
    ...
    >
      <el-form-item prop="proxyPassValue" label="反向代理地址" label-width="140px">
